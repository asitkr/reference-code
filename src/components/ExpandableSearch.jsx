...
export default function ExpandableSearch() {
  ...
  const handleSearch = (value) => {
    ...
    if (value.trim() === "") {
      setResults([]);
      setHovered(false); // collapse when cleared
    } else {
      const filtered = items.filter((item) =>
        item.toLowerCase().includes(value.toLowerCase())
      );
  ...
  return (
    <div className="relative w-fit">
      ...
      <div
        className={`flex flex-row-reverse items-center justify-center h-12 rounded-full bg-black transition-all duration-300 ease-in-out overflow-hidden ${
          hovered || query ? "w-56" : "w-12"
        }`}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => {
          if (!query) setHovered(false); // collapse only if input empty
        }}
      >
        {/* Search / Clear Icon */}
        <div className="px-2 cursor-pointer">
          {query ? (
            <X
              size={20}
              className="text-white"
              onClick={() => handleSearch("")} // clear and collapse
            />
          ) : (
            <Search size={20} className="text-white" />
          )}
        ...
        <input
          ...
          onChange={(e) => handleSearch(e.target.value)}
          className={`bg-transparent text-white outline-none placeholder-white/70 transition-all duration-300 ease-in-out ${
            hovered || query ? "w-40 px-2" : "w-0 px-0"
          }`}
        />
      </div>
